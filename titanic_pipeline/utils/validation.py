...
class DataValidator:
    ...
    def validate_data_schema(
        ...
        if extra_cols:
            ...
            logger.warning(
                f"⚠️  Extra columns in {dataset_name}: {extra_cols}"
            )
    ...
    def validate_data_integrity(self, df: pd.DataFrame, dataset_name: str) -> Dict[str, Any]:
        ...
        for col in numeric_cols:
            ...
            if inf_count > 0:
                ...
                logger.warning(
                    f"⚠️  Infinite values in {dataset_name}.{col}: {inf_count}"
                )
        ...
        for col in numeric_cols:
            if df[col].var() == 0:
                ...
                logger.warning(
                    f"⚠️  Zero variance column in {dataset_name}: {col}"
                )

        # Check for duplicate rows
        if integrity_report["duplicate_rows"] > 0:
            msg = f"⚠️  Duplicate rows in {dataset_name}: {integrity_report['duplicate_rows']}"
            logger.warning(msg)
