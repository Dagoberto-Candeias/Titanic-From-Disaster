...
def create_sample_data():
    ...
    feature_cols = [f'feature_{i}' for i in range(n_features)] + ['Pclass', 'Sex', 'Embarked']
    ...
    model_results = {
        'RandomForest': {
            ...
            'mean_recall': 0.79,
            'mean_f1': 0.80,
            'trained_model': RandomForestClassifier(random_state=42, n_estimators=10).fit(X_train, y_train)
        },
        'LogisticRegression': {
            'mean_score': 0.78,
            'std_score': 0.03,
            ...
            'mean_recall': 0.72,
            'mean_f1': 0.73,
            'trained_model': LogisticRegression(random_state=42, max_iter=100).fit(X_train, y_train)
        }
    }
...
def test_standalone_functions():
    ...
    try:
        generate_reports(model_results, feature_cols, X_train, y_train)
    ...
    try:
        ...
        generate_permutation_importance(model, X_train, y_train, feature_cols, model_name='RandomForest')
...
def main():
    ...
    for dir_path in ['output/relatorios', 'output/graficos', 'output/models', 'output/reports', 'output/changelog']:
